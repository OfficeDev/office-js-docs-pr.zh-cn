...
1. 在 GitHub 找到想要参与的文章。 
    ...
    如果此文章在 MSDN 中，请选择“**为改进此内容做贡献**”部分的“**建议并提交更改**”链接，你将转到 GitHub 上的同一篇文章。
...
2. 拥有帐户后，在计算机上安装 Git。 按[“安装 Git 教程”](https://help.github.com/articles/set-up-git/)中的步骤执行操作。
...
签署此 CLA 并未授予你操纵主存储库的权利，但确实表示 Office 开发人员和 Office 开发人员内容发布团队可以查看并批准你提供的内容。针对你提供的内容，你将获得相应的奖励。
...
1.  建立 GitHub 帐户以参与此项目。 如果尚未执行此操作，请转到 [GitHub](https://github.com/join) 并立即执行操作。
2.  在计算机上安装 Git。 按[“安装 Git 教程”] [Set Up Git]中的步骤执行操作。
3.  针对此存储库创建自己的分叉。 为此，在页面顶部，选择 **“分叉”** 按钮。
...
**重要说明：** 为了简化工作流并降低合并冲突发生机率，每个分支只能包含一个概念/文章。 适用于新分支的内容包括：
...
中的文件是在本地存储库中创建的新分支的副本，目前正在运行。`C:\Users\<yourusername>\<repo name>` 提交更改前，在此文件夹的任何更改都不会影响本地存储库。 若要向本地存储库提交更改，在 GitBash 中键入以下命令：

    git add .
    git commit -v -a -m "<Describe the changes made in this commit>"

命令将更改添加到暂存区域，以准备将更改提交到存储库。`add` 命令后面的点号指明要暂存所有已添加或修改的文件，同时以递归方式查看子文件夹。`add` （如果不希望提交所有更改，可以添加特定的文件）。 还可以撤消提交。 若要获取帮助，请键入 `git add -help` 或 `git status`。）

命令将暂存更改应用到存储库。`commit` 开关表示命令行中有提交注释。`-m` 可以忽略 -v 和 -a 开关。 -v 开关用于命令的详细输出，-a 执行添加命令已执行的操作。
...
1.  在 Git Bash 命令提示符处，键入 `git push origin <new branch name>`。 在本地存储库中，`origin` 是指本地存储库复制自的 GitHub 存储库。 此命令将新分支的当前状态（包括上述步骤中的所有提交）推送到 GitHub 分支。
...
并对内容进行验证。 （`checkout` 命令将 `C:\Users\<yourusername>\microsoft-graph-docs` 中的文件更新为 X2 分支的当前状态。）签出新分支后，便可以照常更新并提交内容。 但是为了避免误在已合并的分支 (X) 进行操作，最好将其删除（请参阅下列“**删除分支**”部分）。

#### <a name="delete-a-branch"></a>删除分支

成功将更改并入主存储库后，将已使用的分支删除，因为将无需再使用该分支了。其他任何操作都应在新分支中完成。  

#### <a name="to-delete-a-branch"></a>删除分支的操作

1.  在 Git Bash 命令提示符处，键入 `git checkout master`。 这样可以确保所在分支不是要删除的（否则，操作将被禁）。
2.  接下来，在命令提示符处，键入 `git branch -d <branch name>`。 此操作只会删除计算机上已成功合并到上游存储库的分支。 （可以使用 `–D` 标记重写此行为，但请先确保自己需要这么做。）
3.  最后，在命令提示符处，键入 `git push origin :<branch name>`（冒号前有一个空格，冒号后没有空格）。  这将删除 github 分叉中的分支。  
...
此存储库中的所有文章都使用 Markdown。 [Markdown 主页](http://daringfireball.net/projects/markdown/ 
)中有完整介绍（并列出了所有语法）。
...
社区成员**必须签署参与许可协议 (CLA)，才能通过提交大量内容参与此项目**。 只需完成并提交此文档一次。 请仔细查看该文档。 可能要求你让你的员工签署此文档。

### <a name="what-happens-with-my-contributions"></a>我的参与内容会接受哪些处理？
...
* 若要详细了解如何使用 Git 和 GitHub，请先查看[“GitHub 帮助”部分] [GitHub 帮助]。
