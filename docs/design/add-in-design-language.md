# <a name="office-add-in-design-language"></a>Office 外接程序设计语言
...
|![特大文本图像](../images/add-in-typeramp-title.png)|.ms-font-xl |21 像素 |Segoe Light | <ul><li>此类匹配 Office 应用程序的任务窗格标题。</li><li>请谨慎使用以避免出现平面版式层次结构。</li><li>通常用作对话框、页面或内容标题等顶级元素。</li><li></ul> |
...
|![特大文本图像](../images/add-in-typeramp-body.png)|.ms-font-m |14 像素 |Segoe Regular |*通常用作外接程序中的正文文本。 |
|![特大文本图像](../images/add-in-typeramp-caption.png)|.ms-font-xs |11 像素 | Segoe Regular |*通常由行、标题或字段标签用于时间戳等二级或三级文本。 |
|![特大文本图像](../images/add-in-typeramp-annotation.png)|.ms-font-mi |10 像素 |Segoe Semibold |*应少使用类型斜坡中的最小步长。它仅供不需要辨别的情况使用。 |
> 这些基类中不包含文本颜色。在白色背景上将 Fabric 的“neutral primary”用于大多数文本。
...
>  对于邮件和任务窗格外接程序，请使用 [Context.officeTheme](https://dev.office.com/docs/reference/shared/office.context.officetheme.htm) 元素匹配 Office 应用程序的主题。此 API 当前仅在 Office 2016 中可用。

> 对于 PowerPoint 内容外接程序，请参阅[在 PowerPoint 外接程序中使用 Office 主题](https://dev.office.com/docs/add-ins/powerpoint/use-document-themes-in-your-powerpoint-add-ins.htm)。
...
* 请注意色盲。使用多个颜色来表示交互性和层次结构。
* 请参阅[图标准则](design-icons.md)以了解更多关于使用 Office 图标调色板设计外接程序命令图标的信息。
...
有关布局指南，请参阅“任务窗格”[](task-pane-add-ins.md)、“内容”[](content-add-ins.md)和“对话框”[](dialog-boxes.md)。有关如何将 Office UI Fabric 组件装配到通用布局和用户体验流的详细信息，请参阅 [UX 设计模式模板](ux-design-patterns.md)。
...
Office 功能区界面具有标准的视觉样式。如果你正在为 Office 功能区设计外接程序命令，请遵循我们的[图标准则](design-icons)。这可以确保一致性并熟悉各个 Office 应用程序。这些准则将有助于你为解决方案设计一组适合作为 Office 固有部分的 PNG 资产。
...
* 不要误用不相关的操作隐喻。将相同的视觉对象用于不同行为或概念可能导致用户困惑。

## <a name="animation"></a>动画
UI 元素、控件和组件通常具有需要过渡、运动或动画的交互行为。UI 界面元素之间运动的共同特征定义设计语言的动画方面。Office 的重点是工作效率，因此 Office 动画语言支持帮助客户完成工作的目标。它可在性能响应、可靠设计和细节带来的喜悦之间实现平衡。

Office UI fabric 包含一个动画库来控制 HTML 容器中的运动。使用它可以在 Office 中无缝整合。它将帮助你创建感受多于观察的体验。动画 CSS 类提供方向性、进入/退出和强化 Office 心理模型的持续时间细节，并为客户提供机会了解如何与外接程序进行交互。 

如果外接程序有其自己的动画语言，则使用它。寻找与 Office 集成的同时保留品牌动画的机会。请注意，不要干扰 Office 中常见的运动模式或与其发生冲突。避免创建用作装饰只会分散客户注意力的体验。

将下列一般原则应用于动画：

* 应潜意识里感受和体验动画，以免阻碍任务完成。
* 避免预期、反弹、橡皮筋或其他模拟自然世界物理学的效果。
* 设计元素以巩固层次结构和心理模型。
* 使用运动指导用户并提供完成任务所需的针对关键元素的组合焦点。 
* 请考虑触发元素的起源。使用运动在操作和结果 UI 之间创建链接。
* 选择动画时，请考虑内容的基调和用途。以不同于探索性导航的方式处理重要消息。
