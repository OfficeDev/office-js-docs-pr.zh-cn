...
![已调用基本和修饰符区域网格示意图。](../images/monolineicon3.png)
...
每个大小图标在图标周围都有指定数量的填充。 基本元素保留在填充内，但修饰符应向上扩展到画布边缘，扩展到填充之外到图标边框的边缘。 下图显示了用于每个图标大小的推荐填充。
...
间隙的大小应该与用于该大小的线粗细的宽度相同。 如果制作 16 像素的图标，间隙宽度为 1px，如果是 48 像素的图标，间隙应为 2px。 以下示例显示一个 32 像素的图标，修饰符和基础基之间的间隙为 1px。
...
调色板专为简化和辅助功能设计。 它包含 4 种中性颜色以及蓝色、绿色、黄色、红色和紫色的两种变体。 橙色有意不包含在单声道图标调色板中。 每种颜色旨在以本节所述的特定方式使用。
