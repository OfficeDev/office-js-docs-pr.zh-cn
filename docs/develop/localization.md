...
description: 使用 Office JavaScript API 根据 Office 应用程序的区域设置确定区域设置和显示字符串，或根据数据区域设置解释或显示数据。
ms.date: 07/18/2022
...
ms.locfileid: "66889462"
---
# <a name="localization-for-office-add-ins"></a>Office 加载项的本地化

您可以实现适合 Office 外接程序的任何本地化方案。 Office 外接程序平台的 JavaScript API 和清单架构提供了一些选择。 可以使用 Office JavaScript API 根据 Office 应用程序的区域设置确定区域设置和显示字符串，或者根据数据区域设置解释或显示数据。 可以使用清单指定区域设置特定的加载项文件位置和描述性信息。 也可以使用 Microsoft Ajax 脚本支持全球化和本地化。
...
- [Context.displayLanguage][displayLanguage] 指定 Office 应用程序用户界面的区域设置 (或语言) 。 以下示例验证 Office 应用程序是否使用 en-US 或 fr-FR 区域设置，并显示特定于区域设置的问候语。
...
- [Context.contentLanguage][contentLanguage] 指定数据的区域设置（或语言）。 扩展最后一个代码示例，而不是检查 [displayLanguage] 属性，分配 `myLanguage` [contentLanguage] 属性的值，并使用同一代码的其余部分根据数据区域设置显示问候语。
...
> [!NOTE]
> 如需针对一个语系内的多个区域进行本地化，例如 `de-de` 和 `de-at`，则建议对各个区域使用独立的 `Override` 元素。 在这种情况下 `de`，不支持在 Office 客户端应用程序和平台的所有组合中单独使用语言名称。
...
以下示例为 [Description] 元素应用区域设置替代。 它首先指定默认区域`en-us`设置和英语说明，然后指定具有区域设置的法语说明`fr-fr`的 [Override] 语句。
...
## <a name="localize-extended-overrides"></a>本地化扩展重写

Office 外接程序的某些可扩展性功能（如键盘快捷方式）配置有托管在服务器上的 JSON 文件，而不是外接程序的 XML 清单。 本部分假定你熟悉扩展替代。 请参阅使用清单和 [ExtendedOverrides](/javascript/api/manifest/extendedoverrides) 元素[的扩展替代](extended-overrides.md)。

`ResourceUrl`使用 [ExtendedOverrides](/javascript/api/manifest/extendedoverrides) 元素的属性将 Office 指向本地化资源的文件。 示例如下。
...
然后，扩展替代文件使用令牌而不是字符串。 资源文件中的令牌名称字符串。 下面是一个示例，用于将键盘快捷方式分配给 (在其他) 中定义的显示加载项任务窗格的函数。 请注意此标记：

- 此示例不太有效。  (我们在下面添加了所需的附加属性。) 
- 令牌必须具有 **${resource 格式。*资源名称*}**。
...
资源文件也采用 JSON 格式，具有按区域设置划分为子属性的顶级 `resources` 属性。 对于每个区域设置，将为扩展替代文件中使用的每个令牌分配一个字符串。 下面是一个包含字符串的`en-us``fr-fr`示例。 在此示例中，键盘快捷方式在两个区域设置中都是相同的，但情况并不总是如此，尤其是当你为具有不同字母表或写入系统的区域设置进行本地化时，因此键盘也不同。
...
文件中没有`default`与这些和`fr-fr`部分对等互连的`en-us`属性。 这是因为在 Office 主机应用程序的区域设置与资源文件中的任何 *ll-cc* 属性不匹配时使用的默认字符串 *必须在扩展重写文件本身中定义*。 直接在扩展替代文件中定义默认字符串可确保 Office 在 Office 应用程序的区域设置与清单) 中指定的外接程序 (的默认区域设置匹配时不会下载资源文件。 下面是使用资源令牌的扩展替代文件的前面示例的更正版本。
...
可以使用 **[displayLanguage]** 属性获取 Office 客户端应用程序的用户界面的区域设置。 然后，可以以与 Office 应用程序的当前区域设置一致的格式显示日期和时间值。 执行上述操作的一种方法是准备一个指定日期/时间显示格式的资源文件以用于 Office 外界程序支持的各个区域设置。 在运行时，外接程序可以使用资源文件，并将适当的日期/时间格式与从 **[displayLanguage]** 属性获取的区域设置相匹配。

可以使用 [contentLanguage] 属性获取 Office 客户端应用程序数据的区域设置。 基于此值，可以正确地解读或显示日期/时间字符串。 例如，`jp-JP` 区域设置将数据/时间值表示为 `yyyy/MM/dd`，而 `fr-FR` 区域设置则表示为 `dd/MM/yyyy`。
...
若要运行提供的示例代码，请在计算机上将 Office 配置为使用其他语言，以便通过切换用于在菜单和命令中显示的语言、进行编辑和校对或同时进行两者兼有来测试加载项。
...
需要创建 Visual Studio 2019 Office 外接程序项目。

> [!NOTE]
> 如果尚未安装 Visual Studio 2019，请参阅 [Visual Studio IDE 页](https://visualstudio.microsoft.com/vs/) 以获取下载说明。 在安装过程中，你需要选择 Office/SharePoint 开发工作负载。 如果以前安装了 Visual Studio 2019，[请使用Visual Studio 安装程序](/visualstudio/install/modify-visual-studio/)确保安装了 Office/SharePoint 开发工作负载。

1. 选择“**创建新项目**”。

1. 使用搜索框，输入“**加载项**”。 选择“**Word Web 外接程序**”，然后选择“**下一步**”。

1. 将项目命名为 **WorldReadyAddIn** ，然后选择 **“创建**”。
...
1. 例如，如果您将 Office 2013 的显示语言从英语切换到西班牙语，然后运行加载项，加载项的显示名称和说明将用本地化文本显示。

#### <a name="lay-out-the-add-in-ui"></a>布局加载项 UI

1. 在 Visual Studio 的“解决方案资源管理器”中，选择“Home.html”。

1. 在 Home.html 中，将 `<body>` 元素替换为以下 HTML，并保存文件。
...
### <a name="add-the-resource-file-to-the-add-in-project"></a>将资源文件添加到外接程序项目

1. 在 Visual Studio 的“解决方案资源管理器”中，右键单击“WorldReadyAddInWeb”项目并选择“添加” > “新项目”。

1. 在“添加新项目”对话框中，选择“JavaScript 文件”。

1. 输入文件名 **UIStrings.js**，然后选择“添加”。

1. 将以下代码添加到 UIStrings.js 文件，然后保存文件。
...
如果要根据在 Office 客户端应用程序的菜单和命令中显示的语言更改外接程序的本地化，请使用 **Office.context.displayLanguage** 属性获取该语言的区域设置。 例如，如果应用程序语言使用西班牙语在菜单和命令中显示， **则 Office.context.displayLanguage** 属性将返回语言代码 es-ES。

如果要根据用于编辑文档内容的语言更改外接程序的本地化，请使用 **Office.context.contentLanguage** 属性获取该语言的区域设置。 例如，如果应用程序语言使用西班牙语编辑文档内容， **则 Office.context.contentLanguage** 属性将返回语言代码 es-ES。

知道应用程序使用的语言后，可以使用 **UIStrings** 获取与应用程序语言匹配的本地化字符串集。

用以下代码替换 Home.js 文件中的代码。 代码显示如何根据应用程序的显示语言或应用程序的编辑语言更改Home.html上的 UI 元素中使用的字符串。

> [!NOTE]
> 要根据编辑所使用的语言在更改加载项本地化之间进行切换，请取消注释代码行 `const myLanguage = Office.context.contentLanguage;` 并注释掉代码行 `const myLanguage = Office.context.displayLanguage;`
...
若要测试本地化的外接程序，请更改用于在 Office 应用程序中显示或编辑的语言，然后运行加载项。
...
2. 在“**选择显示语言**”下，选择想要显示的语言，例如西班牙语，然后选择向上箭头键将西班牙语移至列表中的第一个位置。 或者，若要更改用于编辑的语言，请在 **“选择编辑语言**”下选择要用于编辑的语言，例如西班牙语，然后选择 **“设置为默认值**”。
...
运行后，外接程序 UI 中的字符串将更改为与应用程序使用的语言匹配，如下图所示。
