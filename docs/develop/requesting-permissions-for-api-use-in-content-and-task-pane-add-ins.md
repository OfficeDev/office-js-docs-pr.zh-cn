...
description: 了解在内容或任务窗格加载项清单中声明以指定 JavaScript API 访问级别的不同权限级别。
ms.date: 03/19/2019
...
这些权限指定加载项 [运行时](../testing/runtimes.md) 允许内容或任务窗格加载项在用户插入时使用的 API 子集，然后激活 (信任) 加载项。 若要声明内容或任务窗格加载项所需的权限级别，请在加载项清单的 [Permissions](/javascript/api/manifest/permissions) 元素中指定任一权限文本值。 以下示例要求 **WriteDocument** 权限，仅允许可以对文档进行写入（而非阅读）的方法。
...
|**ReadDocument**|除了 **受限** 权限允许的 API 外，还增加了对读取文档和管理绑定所需的 API 成员的访问权限。这包括使用：<br/><ul><li>
