...
- 使用仅在某些版本的 Office 中可用的 Office JavaScript API。 例如，批量许可的永久Excel 2016版本不支持 Office JavaScript 库中与 Excel 相关的所有 API。
...
- 外接程序具有一项功能，该功能需要 Office JavaScript API 方法，该方法在某些版本的 Office 应用程序（如 Microsoft 365 订阅 Excel）中受支持，但在其他版本中不受支持，例如批量许可的永久Excel 2016。 但外接程序具有其他仅需要在批量许可的永久Excel 2016 *中支持的 Office* JavaScript API 方法的功能。 在此方案中，需要在该版本的Excel 2016上安装加载项，但需要不受支持的方法的功能应对这些用户隐藏。
...
要求集支持因 Office 应用程序、Office 应用程序的版本及其运行的平台而异。 例如，Office 2021之前，Office 的批量许可永久版本不支持 DialogApi 1.2，但所有永久版本都支持 DialogApi 1.1，返回 Office 2013。 你希望您的外接程序可安装在支持其使用的 API 的平台和 Office 版本的每个组合上，因此应始终在清单中指定加载项所需的每个要求集 *的最低* 版本。 本文稍后将介绍有关如何执行此操作的详细信息。
