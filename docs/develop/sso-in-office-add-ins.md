...
`callback` - 可选。接受回调方法分析令牌，用用户 ID 或用“代表”流进入 Microsoft Graph。如果 [AsyncResult](https://docs.microsoft.com/javascript/api/office/office.asyncresult)`.status`  为"成功"，则`AsyncResult.value` 是原始 AAD v. 2.0-被格式化的登陆令牌。
