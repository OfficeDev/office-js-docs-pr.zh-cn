...
本文介绍了如何将 Office 加载项项目中的 JavaScript 文件（Office.js 和特定于应用程序的 .js 文件）和加载项清单验证文件更新到版本 1.1。

## <a name="use-the-most-up-to-date-project-files"></a>使用最新项目文件

如果您使用 Visual Studio 来开发加载项，以使用适用于 Office 的 JavaScript API 的 [最新 API 成员](https://docs.microsoft.com/office/dev/add-ins/reference/what's-changed-in-the-javascript-api-for-office?view=office-js)和 [加载项清单 v1.1 功能](../develop/add-in-manifests.md)（根据 offappmanifest-1.1.xsd 进行了验证），则您需要下载并安装 [Visual Studio 2015 和最新的 Office 开发人员工具](https://www.visualstudio.com/features/office-tools-vs)。

如果您使用文本编辑器或 Visual Studio 以外的 IDE 开发加载项，则您需要针对在加载项的清单中引用的 Office.js 和架构版本，将引用更新到 CDN。
...
对于在适用于 Office 的 JavaScript API v1.1 和加载项清单架构发布之前创建的项目，你可以使用 **NuGet 程序包管理器**更新项目文件，然后更新加载项的 HTML 页以进行引用。 

请注意，更新过程对_每个项目_分别执行，这意味着对于想要在其中使用 Office.js v1.1 的每个加载项项目以及加载项清单架构，您需要重复更新过程。
