...
若要运行使用新的和更新的 API Office.js外接程序清单功能开发的外接程序， 您的客户必须运行 Office 2013 SP1 或更高版本本地产品，以及 SharePoint Server 2013 SP1 和相关服务器产品、Exchange Server 2013 Service Pack 1 (SP1) 或等效联机托管产品：Microsoft 365、SharePoint Online 和 Exchange Online。
...
以下步骤将你的Office.js库文件更新到最新版本。 这些步骤使用 Visual Studio 2019，但它们与早期版本的 Visual Studio 类似。
...
需要执行其他步骤才能完成更新。 在外接程序的 HTML 页的 **head** 标记中，注释掉或删除任何现有的 office.js 脚本引用，并引用更新的 Office JavaScript API 库，如下所示：
...
你不需要 Office JavaScript API 文件的本地副本 (Office.js 和特定于应用的 .js 文件) 来开发引用 CDN for Office.js 的Office 外接程序 (在运行时) 下载必要的文件，但是如果你想要库文件的本地副本，可以使用[NuGet Command-Line](https://docs.nuget.org/consume/installing-nuget)实用工具和 命令下载它们。 `Install-Package Microsoft.Office.js`
...
2. 在外接程序的 HTML 页的 **head** 标记中，注释掉或删除任何现有的 office.js 脚本引用，并引用更新的 Office JavaScript API 库，如下所示：
