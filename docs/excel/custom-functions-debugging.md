...
1. 打开自定义函数根项目[文件夹VS Code。](https://code.visualstudio.com/)
...
## <a name="use-the-vs-code-debugger-for-excel-in-microsoft-edge"></a>使用 VS Code 调试器Excel中的Microsoft Edge

可以使用VS Code在浏览器上的 Excel 调试无 UI Microsoft Edge函数。 若要VS Code开发人员Microsoft Edge，必须安装 Microsoft Edge[的 DevTools Visual Studio Code。](/microsoft-edge/visual-studio-code/microsoft-edge-devtools-extension)
...
2. 在开发人员工具中，使用 **Cmd+P** 或 **Ctrl+P** 或 functions.ts (functions.js **打开** 源代码脚本) 。 
...
3. 如果要在桌面版本的外接程序中启动Excel，请运行以下命令。
    ...
    `npm run start:web -- --document {url}` (，其中 是 Excel `{url}` 或 OneDrive 上的SharePoint) 
...
- `npm run start:desktop`：Excel在桌面上启动加载项，并旁加载加载项。
- `npm run start:web -- --document {url}` (，其中 是 Excel 或 OneDrive 上的 `{url}` SharePoint) ：Excel web 版并旁加载外接程序。
