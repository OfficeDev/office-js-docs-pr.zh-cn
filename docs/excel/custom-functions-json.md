...
我们建议尽可能使用 JSON 自动生成，而不是创建自己的 JSON 文件。 自动生成不易出现用户错误，并且基架文件 `yo office` 已包含此错误。 有关 JSDoc 标记和 JSON 自动生成过程的信息，请参阅自动生成 [自定义函数的 JSON 元数据](custom-functions-json-autogeneration.md)。
