...
借助自定义函数（类似于用户定义的函数 (UDF)），开发人员可以使用加载项向 Excel 添加任何 JavaScript 函数。 然后，用户可以在 Excel 中像访问任何其他本机函数（如 =SUM()）那样访问自定义函数。 本文介绍如何在 Excel 中创建自定义函数。
...
自定义函数（预览）现已可用。 请按照下列步骤试用自定义函数（预览）：

1.  安装适用于 Windows 的 Office 2016 并加入 [Office 预览体验成员](https://products.office.com/zh-CN/office-insider)计划。
2.  克隆 [Excel 自定义函数](https://github.com/OfficeDev/Excel-Custom-Functions)存储库并按照 *README.md* 中的说明在 Excel 中启动加载项。
3.  将 `=CONTOSO.ADD42(1,2)` 键入到任意单元格，然后按“Enter”****以运行自定义函数。
...
注册后，用户即可在所有工作簿（不仅是最初运行加载项的工作簿）中使用自定义函数。 当用户开始键入时，这些函数会在自动完成菜单中显示。
...
-   &lt;`Script`&gt; 元素，该元素是必需的元素，但开发者预览版中未使用它。
-   &lt;`Page`&gt; 元素，该元素链接到加载项的 HTML 页面。 HTML 页面包含对 JavaScript 文件 (*customfunctions.js*) 的&lt;脚本&gt;引用，其中包含自定义函数和注册代码。 HTML 页面是一个隐藏页面，始终不会在 UI 中显示。
...
3.   使用 `setResult` 回调解析此 Promise。 `setResult` 将值发送给 Excel。
...
对于流式处理函数，始终不会在注册代码中指定最后一个参数 `setResult`，且当用户输入此函数时，该参数不会在 Excel 用户的自动完成菜单中显示。 它是一个回调函数，用于将数据从函数传递到 Excel，以更新单元格的值。 若要使 Excel 传递 `setResult` 函数，必须在注册函数时通过将参数 `stream` 设置为 `true` 来声明对流式处理的支持。
...
下面的代码演示之前的温度流式处理函数的实施过程，该函数使用 `savedTemperatures` 变量保存状态。 此代码将演示以下概念：
...
例如，假设函数从 Excel 中存储的温度值区域返回第二个最高温度。 以下函数采用参数 `temperatures`，它是 `Excel.CustomFunctionDimensionality.matrix` 参数类型。
...
-   批处理：用于聚合对同一函数的多个调用，以提升性能。

-   取消：当不再需要使用流式处理函数时（例如，当用户清除单元格时）发出通知。 目前函数无法确定何时停止向单元格写入新值。

-   Excel 目前尚未使用帮助 URL 和参数说明。

-   将使用自定义函数的加载项发布到 Office 应用商店或 Office 365 集中部署。
