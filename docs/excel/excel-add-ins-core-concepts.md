...
由于 **sync()** 是一个返回 promise 的异步操作，因此，（在 JavaScript 中）应始终**返回** promise。 这样做可确保在脚本继续运行之前完成 **sync()** 操作。
...
若要优化性能，在对象上使用 **load()** 方法时，应显式指定要加载的属性和关系。 例如，如果你只想读回区域对象的 **address** 属性，则在调用 **load()** 方法时仅指定该属性。
