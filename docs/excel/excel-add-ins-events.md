...
| `onDeleted`  | 删除对象时发生的事件。 | **WorksheetCollection** |
| `onActivated` | 启用对象时发生的事件。 | **WorksheetCollection**、**Worksheet** |
| `onDeactivated` | 停用对象时发生的事件。 | **WorksheetCollection**、**Worksheet** |
| `onDataChanged` | 更改单元格内数据时发生的事件。 | **Worksheet**、**Table**、**TableCollection**、**Binding** |
...
下面的代码示例为 **Sample** 工作表中的 `onDataChanged` 事件注册事件处理程序。 此代码指定 `handleDataChange` 函数应在工作表中的数据有变化时运行。
