...
扩展 SaaS Web 应用程序，以便客户可以直接从网页打开数据到 Microsoft Excel。 常见方案是客户将处理 Web 应用程序中的数据。 然后，他们将希望将数据复制到 Excel 文档中。 例如，他们可能需要使用 Excel 执行其他分析。 通常，客户需要将数据导出到文件（如 .csv 文件）中，然后将该数据导入 Excel。 他们还必须手动将 Office 加载项添加到文档中。

将步骤数减少为在生成并打开 Excel 文档的网页上单击一次按钮。 还可以在文档中嵌入 Office 外接程序，在文档打开时显示它。 这将确保客户仍可访问应用程序功能。 文档打开后，客户选择的数据和 Office 外接程序已可供他们继续工作。

本文介绍了在您自己的 SaaS Web 应用程序中实现此方案的代码和技术。

## <a name="create-a-new-excel-document-and-embed-an-office-add-in"></a>创建新的 Excel 文档并嵌入 Office 加载项

首先，让我们了解如何从网页创建 Excel 文档，以及如何在文档中嵌入加载项。 [Office OOXML 嵌入](https://github.com/OfficeDev/Office-OOXML-EmbedAddin)外接程序代码示例演示如何将 Script [Lab](https://appsource.microsoft.com/product/office/wa104380862)加载项嵌入新的 Office 文档中。 虽然该示例适用于任何 Office 文档，但本文中我们将重点介绍 Excel 电子表格。 使用以下步骤生成并运行示例。

1. 将示例代码从  https://github.com/OfficeDev/Office-OOXML-EmbedAddin/archive/master.zip 您的计算机提取到文件夹中。
2. 若要生成并运行示例，请按照自述文件 **的项目部分中的** 步骤操作。
...
示例代码使用 OOXML SDK 将 Script Lab 加载项嵌入到选择的 Excel 文档中。 以下信息取自自述文件中"[关于代码"](https://github.com/OfficeDev/Office-OOXML-EmbedAddin/blob/master/README.md)部分。

该文件 **Home.aspx.cs：**

- 提供按钮事件处理程序和基本 UI 操作。
- 使用标准ASP.NET技术上载和下载文件。
- 使用已上传文件的文件扩展名 (xlsx、docx 或 pptx) 确定文件类型。 这一点需要在开始时完成，因为 Open XML SDK 通常具有每种类型的文件不同的 API。
- 调用 **OOXMLHelper** 以验证文件并调用 **AddInEmbedder** 以在文件中嵌入 Script Lab，并设置为自动打开。

该文件 **AddInEmbedder.cs：**

- 提供主业务逻辑，此示例中是嵌入 Script Lab 的方法。
- 根据文件类型调用 OOXML 帮助程序。
...
- 主要包含由 Open XML 2.5 SDK Productivity Tools 生成的代码，这些代码在 [Open XML 2.5 SDK 的链接中提供](/office/open-xml/open-xml-sdk)。
...
- **存储** 值是网络共享 URL;例如 \\ \\ ，"MyComputer \\ MySharedFolder"。 这应该是在 Office 信任中心显示为共享"受信任的目录地址"的确切 URL。
- **Id** 值是加载项清单中的应用程序 ID。
> [!NOTE]
> 有关这些属性的可选值的详细信息，请参阅"自动打开[包含文档的任务窗格"。](../develop/automatically-open-a-task-pane-with-a-document.md)
...
1. 将按钮代码替换为以下图像标记。
    ...
    ```
1. 按 **F5** (**或调试>开始调试) 。** 在加载主页时，你将看到图标显示。
...
如果你的客户使用 OneDrive，我们建议将新文档上载到 OneDrive。 这便于用户查找和处理文档。 让我们创建新的代码示例，并了解如何使用 Microsoft Graph SDK 将新的 Excel 文档上载到 OneDrive。

### <a name="use-a-quick-start-to-build-a-new-microsoft-graph-web-application"></a>使用快速入门构建新的 Microsoft Graph Web 应用程序

1. 转到 [https://developer.microsoft.com/graph/quick-start](https://developer.microsoft.com/graph/quick-start) 并按照步骤创建和打开与 Office 服务交互的快速启动代码示例。
1. 在 **步骤 1：选择语言或平台，ASP.NET** **MVC。** 虽然此过程中的步骤使用 MVC ASP.NET，但步骤遵循适用于任何语言或平台的模式。
1. 在 **步骤 2 中：获取应用 ID 和密码，** 选择 **"获取应用 ID 和密码"。**
1. 登录到 Microsoft 365 帐户。  
1. 在 **"请保存应用密码** "网页上，将应用程序密码保存到文件位置，稍后可在其中检索和使用。
...
1. 在 2019 年 Visual Studio graph-tutorial.sln 文件。
1. 生成并运行解决方案并确认它正常工作。 你应该能够使用日历网页来查看 Microsoft 365 日历。
...
1. 将新的范围 **Files.ReadWrite**   添加到 **ida：AppScopes** 项，以便其外观类似于以下代码：
    ```xml
    <add key="ida:AppScopes" value="User.Read Calendars.Read Files.ReadWrite " />
    ```
1. 打开 **Index.cshtml** 文件。
1. 插入以下 ActionLink 代码以创建将文件上载到 OneDrive 的按钮。
    ```razor
...
现在，你已了解如何将文件上载到 OneDrive，可以重复使用此代码上载创建的任何 Excel 文档。
...
该示例修改现有的 Excel 文档。 更常见的方案是，从网页创建新的 Excel 电子表格。 可以通过提供文件名来查找有关如何在"创建电子表格"文档中创建新电子表格的其他详细信息。 本文演示如何在本地创建文件，但您也可以使用 SpreadsheetDocument.Create 方法上的重载在流中创建文件。

### <a name="read-custom-properties-when-your-add-in-starts"></a>在加载项启动时读取自定义属性

该代码示例使用 OOXML SDK 将代码段 ID 存储在新的 Excel 文档中。 Script Lab 从 Excel 文档中读取代码段 ID，然后在代码段打开时显示该代码段。 您可能需要向自己的外接程序 (（如查询字符串或临时身份验证令牌）发送自定义属性。) 请参阅"保留外接程序状态和设置"，了解有关在外接程序启动时如何读取自定义属性的完整详细信息。
...
- **将自定义查询属性传递到 Office 外接程序**。 生成文档时，将嵌入 Office 外接程序的自定义属性，其中包含检索所有所需数据的查询字符串。 当您的外接程序打开时，它将检索查询、运行查询，并使用 Office JS API 将查询结果插入文档中。

### <a name="working-with-the-ooxml-sdk"></a>使用 OOXML SDK

OOXML SDK 基于 .NET。 如果 Web 应用程序没有 .NET，则需要查找使用 OOXML 的替代方法。

Open XML SDK for JavaScript 提供了 OOXML [SDK 的 JavaScript 版本](https://archive.codeplex.com/?p=openxmlsdkjs)。
