...
:::image type="content" source="../images/pnp-open-in-excel.png" alt-text="网页上Excel按钮的图像，该按钮可打开一个新的 Excel 文档，其中外接程序嵌入并自动打开。":::

扩展 SaaS Web 应用程序，以便客户可以直接在网页中打开其数据Microsoft Excel。 常见方案是客户将处理 Web 应用程序中的数据。 然后，他们希望将数据复制到一个Excel文档中。 例如，他们可能需要使用 Excel。 通常，客户需要将数据导出到文件（如 .csv 文件）中，然后将该数据导入Excel。 他们还必须手动将Office加载项添加到文档中。

将步骤数减少为生成文档并打开文档的网页上的单个Excel单击。 您还可以在文档中Office外接程序，在文档打开时显示它。 这将确保客户仍可访问你的应用程序功能。 当文档打开时，客户选择的数据和Office加载项可供他们继续工作。
...
示例代码使用 OOXML SDK 将Script Lab加载项嵌入Excel文档。 以下信息来自自述 [**文件的关于**](https://github.com/OfficeDev/Office-OOXML-EmbedAddin/blob/master/README.md)代码部分。
...
- 调用 **OOXMLHelper** 以验证文件并调用 **AddInEmbedder** 以在Script Lab嵌入文件并设置为自动打开。
...
- 提供所有详细的 OOXML 操作。
- 使用用于验证文档文件Office技术，该技术只是调用 **其上的 Document.Open** 方法。 如果文件无效，该方法将引发异常。
...
- **Store** 值是网络共享 URL;例如 \\ \\ ，"MyComputer \\ MySharedFolder"。 这应该是在共享信任中心显示为共享受信任目录Office URL。
...
1. 在 **"步骤 1： 选择语言或** 平台"中，选择 **"ASP.NET MVC"。** 虽然此过程中的步骤使用 ASP.NET MVC 选项，但步骤遵循适用于任何语言或平台的模式。
...
1. 在 2019 年 10 月中打开 graph-tutorial.sln Visual Studio文件。
...
1. 插入以下代码以调用 Microsoft Graph API，以在 OneDrive 上创建新OneDrive。
...
该代码示例使用 OOXML SDK 将代码段 ID Excel文档。 Script Lab从文档读取代码段 ID Excel代码段 ID，然后在代码段打开时显示该代码段。 您可能需要将自定义属性发送到您自己的外接程序 (如查询字符串或临时身份验证令牌。) 请参阅持久化外接程序状态和设置，了解有关在外接程序启动时如何读取自定义属性的完整详细信息。
