...
扩展 SaaS Web 应用程序，以便客户可以直接从网页中打开数据Microsoft Excel。 一种常见方案是客户将处理 Web 应用程序中的数据。 然后，他们希望将数据复制到一个Excel文档中。 例如，他们可能需要使用 Excel。 通常，客户需要将数据导出到文件（如 .csv 文件）中，然后将该数据导入Excel。 他们还必须手动将Office加载项添加到文档中。

将步骤数减少为生成文档并打开文档的网页上的一次Excel操作。 您还可以在文档中Office外接程序，在文档打开时显示它。 这将确保客户仍可访问你的应用程序功能。 文档打开后，客户选择的数据和Office加载项可供他们继续工作。
...
首先，让我们了解如何从网页Excel文档，以及如何在文档中嵌入加载项。 Office [OOXML 嵌入](https://github.com/OfficeDev/Office-OOXML-EmbedAddin)外接程序代码示例演示如何将 Script Lab[外接程序](https://appsource.microsoft.com/product/office/wa104380862)嵌入新的 Office 文档中。 尽管该示例适用于Office文档，但我们将重点介绍本文Excel电子表格。 使用以下步骤生成并运行示例。
...
- 提供所有详细的 OOXML 操作。
- 使用用于验证文档文件Office技术，这仅仅是 **为了调用其上的 Document.Open** 方法。 如果文件无效，该方法将引发异常。
...
- **Id** 值是应用的 Microsoft AppSource 资产 ID Script Lab。
...
最佳做法是使用 Fluent UI 来帮助用户在 Microsoft 产品之间转换。 应始终使用Office图标来指示Office网页启动哪个应用程序。 让我们修改示例代码，以使用 Excel 图标指示它启动 Excel 应用程序。
...
如果你的客户使用 OneDrive，我们建议将新文档OneDrive。 这使用户更易于查找和处理文档。 让我们创建新的代码示例，并了解如何使用 Microsoft Graph SDK 将新的 Excel 文档上载到OneDrive。
