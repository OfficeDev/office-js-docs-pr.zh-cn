# <a name="create-add-in-commands-in-your-manifest-for-excel-word-and-powerpoint"></a>在清单中创建 Excel、Word 和 PowerPoint 的加载项命令
...
下图是对清单中的加载项命令元素的概述。 
![清单中的加载项命令元素概述](office-js-docs/images/080da303-51c4-4882-b74a-7ba11517c0ad.png)
 
## <a name="step-1-start-from-a-sample"></a>第 1 步：从示例入手

强烈建议从 [Office 外接程序命令示例](https://github.com/OfficeDev/Office-Add-in-Command-Sample)中提供的示例之一入手。 或者，可以按照本指南中的步骤创建自己的清单。 可以使用 Office 加载项命令示例网站中的 XSD 文件验证你的清单。 使用加载项命令前，请确保已阅读 [Excel、Word 和 PowerPoint 的加载项命令](https://dev.office.com/docs/add-ins/design/add-in-commands)。
...
## <a name="step-3-add-versionoverrides-element"></a>第 3 步：添加 VersionOverrides 元素
...
|**Requirements** <br/> |可选。指定外接程序要求的最低要求集和 Office.js 的版本。此子级 **Requirements** 元素替代清单中父级部分中的 **Requirements** 元素。有关详细信息，请参阅[指定 Office 主机和 API 要求](https://dev.office.com/docs/add-ins/overview/specify-office-hosts-and-api-requirements)。  <br/> |
...
**重要说明**  请确保你的 JavaScript 代码调用 `Office.initialize`。 
...
下面的代码说明了如何实现 **FunctionName** 所使用的函数。
...
## <a name="step-6-add-extensionpoint-elements"></a>步骤 6：添加 ExtensionPoint 元素
**ExtensionPoint** 元素定义加载项命令应在 Office UI 中的哪个位置出现。可以使用以下 **xsi:type** 值定义 **ExtensionPoint** 元素：
...
**重要说明**  对于包含 ID 属性的元素，请务必提供唯一的 ID。 建议将你的公司名称用于此 ID。 例如，使用以下格式。 > <CustomTab id="mycompanyname.mygroupname"> 
...
|**Control** <br/> |每个组需要至少一个控件。**Control** 元素可以是一个**按钮**，也可以是一个**菜单**。使用**菜单**指定按钮控件的下拉列表。目前，仅支持“按钮”和“菜单”。 <br/> 请参阅[按钮控件](https://dev.office.com/reference/add-ins/manifest/control)和[菜单控件](https://dev.office.com/reference/add-ins/manifest/control)各节了解详细信息。 <br/> **注意** 建议逐一添加 **Control** 元素和相关的 **Resources** 子元素，以便于进行故障排除。          |
...
## <a name="step-7-add-the-resources-element"></a>第 7 步：添加 Resources 元素
...
在 Excel 和 Word 中，可以使用默认的 Office UI 选项卡在功能区上添加加载项命令。 下表列出了可以用于 **OfficeTab** 元素的 **id** 属性的值。 这些 Tab 值区分大小写。
...
-  [Excel、Word 和 PowerPoint 的加载项命令](https://dev.office.com/docs/add-ins/design/add-in-commands-for-excel-and-word-preview)
      
-  [在清单中定义加载项命令](https://dev.office.com/docs/add-ins/develop/define-add-in-commands)  
