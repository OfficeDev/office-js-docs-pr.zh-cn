...
2. 使用 Yeoman 生成器生成加载项清单文件。 运行下面的命令，再回答如下所示的提示。
...
对于本快速入门，可以使用**适用于 Office 外接程序的 Yeoman 生成器**提供的证书。 由于已在全局范围内安装了生成器（作为此快速入门的**先决条件**的一部分），因此只需将证书从全局安装位置复制到应用程序文件夹即可。 下面逐步介绍如何完成此过程。
