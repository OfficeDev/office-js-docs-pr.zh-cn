# <a name="build-your-first-powerpoint-add-in"></a>???? PowerPoint ???

??????????? jQuery ? Office JavaScript API ?? PowerPoint ????
...
2. ??Visual C#?****??Visual Basic?****?????????????Office/SharePoint?****????????****?????PowerPoint Web ????****??????? 
...
4. ??? `DefaultValue` ???????`Description` ??????A task pane add-in for PowerPoint?****?
...
1. ?? Visual Studio ????? F5 ???????****???? PowerPoint??????? PowerPoint ???????????????????****?????????????? IIS ??
...
3. ??? `DefaultValue` ???????`Description` ??????A task pane add-in for PowerPoint?****?
...
1. ?????????????????????? PowerPoint ????????
    ...
    - PowerPoint Online?[? Office Online ???? Office ???](../testing/sideload-office-add-ins-for-testing.md#sideload-an-office-add-in-on-office-online)
...
> [PowerPoint ??????](../tutorials/powerpoint-tutorial.yml)
