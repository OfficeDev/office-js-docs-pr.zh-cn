...
例如，可以定义为 `options.asyncContext` 包含一个或多个键值对的 JSON 对象。 有关向异步方法传递可选参数的更多示例，Office外接程序平台中的异步Office[外接程序](../develop/asynchronous-programming-in-office-add-ins.md#pass-optional-parameters-to-asynchronous-methods)。以下示例演示如何使用 参数`asyncContext`将 2 个参数传递给回调方法。
...
> [!NOTE]
> 附件 ID 仅在同一会话中有效，不保证跨会话映射到同一附件。 会话何时结束的示例包括用户何时关闭外接程序，或者用户开始在内嵌表单中撰写，随后弹出内嵌表单以在单独的窗口中继续。

### <a name="attach-a-file"></a>附加文件

您可以使用 方法并 `addFileAttachmentAsync` 指定文件的 URI，将文件附加到撰写窗体中的邮件或约会。 您也可以使用 方法， `addFileAttachmentFromBase64Async` 但将 base64 字符串指定为输入。 如果文件受保护，您可以包括相应的标识或身份验证令牌作为 URI 查询字符串参数。 Exchange 将向 URI 发出调用以获取附件，保护文件的 Web 服务将需要使用令牌作为进行身份验证的一种方式。
...
与 、 `addFileAttachmentAsync`和 `addItemAttachmentAsync``getAttachmentsAsync` 方法类似`removeAttachmentAsync`，是一种异步方法。 您应提供一个回调方法，以使用 output 参数对象 `AsyncResult` 检查状态和任何错误。 您还可以使用可选参数将任何其他参数传递给回调 `asyncContext` 方法。

以下 JavaScript 函数 `removeAttachment`将继续扩展上述示例，并从正在撰写的电子邮件或约会中删除指定的附件。 此函数将要删除的附件的 ID 作为实参。 可以在成功调用 、 或 方法后获取附件的 ID`addFileAttachmentAsync``addFileAttachmentFromBase64Async``addItemAttachmentAsync`，并`removeAttachmentAsync`可在后续方法调用中使用它。 您还可以调用 (`getAttachmentsAsync` 集 1.8 中引入) ，获取该外接程序会话的附件及其 ID。
