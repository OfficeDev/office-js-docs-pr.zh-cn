---
title: 比较Outlook Mac Outlook中的外接程序支持
...
您可以在 Mac 上的 Outlook 上创建和运行 Outlook 加载项，就像在其他客户端（包括 Outlook 网页版、Windows、iOS 和 Android）中一样，而无需为每个客户端自定义 JavaScript。 从外接程序到 JavaScript API 的Office调用通常以相同方式工作，下表中描述的区域除外。

有关详细信息，请参阅[部署和安装 Outlook 外接程序以进行测试](testing-and-tips.md)。

有关新 UI 支持的信息，请参阅新 Mac UI Outlook[外接程序支持](#add-in-support-in-outlook-on-new-mac-ui-preview)。
...
| 项目自定义属性 | 如果网络出现故障，外接程序仍可以访问缓存的自定义属性。 | 由于Outlook Mac 上的加载项不会缓存自定义属性，因此，如果网络关闭，加载项将无法访问它们。 |
...
我们鼓励你预览Outlook Mac UI（版本 16.38.506 中提供）上的预览版本。 若要详细了解如何试用，请参阅 Outlook for Mac [- Insider Fast 内部版本发行说明](https://support.microsoft.com/office/d6347358-5613-433e-a49e-a9a0e8e0462a)。
