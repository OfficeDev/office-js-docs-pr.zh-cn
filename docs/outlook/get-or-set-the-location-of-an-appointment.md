...
可以使用 API `enhancedLocation` 获取和设置约会的位置。 位置字段支持多个位置，对于每个位置，显示名称、类型和会议室电子邮件地址 (（如果适用) ）。 有关支持的位置类型，请参阅[LocationType。](/javascript/api/outlook/office.mailboxenums.locationtype)
