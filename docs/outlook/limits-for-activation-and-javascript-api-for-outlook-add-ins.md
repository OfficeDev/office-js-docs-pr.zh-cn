...
若要详细了解加载项在何处激活和不激活，请参阅"加载项概述"页的"Outlook[](outlook-add-ins-overview.md#mailbox-items-available-to-add-ins)加载项可用的邮箱项目"部分。
...
- `ItemHasKnownEntity`如果在[或 ItemHasRegularExpressionMatch](../reference/manifest/rule.md#itemhasregularexpressionmatch-rule) 规则中使用正则表达式，请注意通常适用于任何 Outlook 应用程序的以下限制和准则，以及表 1、2 和 3 中介绍的限制和准则，这些限制和准则因应用程序不同而不同。
...
|默认情况下，外接程序的所有正则表达式的计算时间限制为 1 秒。 超出此限制将导致最多重新计算 3 次。 超过重新计算限制后，Outlook富客户端会禁止外接程序在任何一个客户端中的同一邮箱Outlook运行。<br/><br/>管理员可以使用 和 注册表项替代`OutlookActivationAlertThreshold``OutlookActivationManagerRetryLimit`这些评估限制。|不支持与 Outlook 富客户端中相同的资源监视或注册表设置。 但是，对于在富客户端上需要大量评估Outlook正则表达式的外接程序在所有 Outlook 客户端上禁用。|
...
||Outlook 富客户端|Outlook 网页版或移动设备版|
|:-----|:-----|:-----|
|**返回的匹配项的顺序**|假定`getRegExMatches`在富客户端中，对同一项目应用的同一正则表达式Outlook与在移动设备或移动设备上Outlook 网页版匹配项。|假定`getRegExMatches`在富客户端中返回匹配项Outlook与在移动设备Outlook 网页版不同。|
...
|正在提取已知实体|2000 个字符|[item.getEntities](../reference/objectmodel/preview-requirement-set/office.context.mailbox.item.md#methods) 方法<br/> <br/>[item.getEntitiesByType](../reference/objectmodel/preview-requirement-set/office.context.mailbox.item.md#methods) 方法<br/> <br/>[item.getFilteredEntitiesByName](../reference/objectmodel/preview-requirement-set/office.context.mailbox.item.md#methods) 方法|在项目正文上提取常见实体的 Exchange Server 限制。 Exchange Server 将忽略超过该限制的实体。 请注意，此限制与外接程序是否使用规则无关 `ItemHasKnownEntity` 。|
|Exchange Web 服务|1 MB 字符数|[mailbox.makeEwsRequestAsync](../reference/objectmodel/preview-requirement-set/office.context.mailbox.md#methods) 方法|请求或呼叫响应的限制 `Mailbox.makeEwsRequestAsync` 。|
...
|设置主题|255 个字符|[Mailbox.displayNewAppointmentForm](../reference/objectmodel/preview-requirement-set/office.context.mailbox.md#methods) 方法<br/><br/> [Subject.setAsync](/javascript/api/outlook/office.subject#outlook-office-subject-setasync-member(1)) 方法|新的约会窗体中的主题限制，或设置约会或邮件主题的限制。|
