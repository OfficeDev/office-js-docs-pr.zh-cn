...
> [!IMPORTANT]
> [AppSource](https://appsource.microsoft.com) 中的加载项不允许使用 Onsend 功能。
...
触发发送事件时，在 Outlook 客户端完成验证。 如果验证失败，系统将阻止发送该项目，并在信息栏中显示一条错误消息，提示用户采取操作。  
...
- `NoSend` &ndash; 一个字符串，即开发人员指定用于引用通知邮件的密钥。 可用于在以后修改此邮件。 密钥长度不得超过 32 个字符。
