...
> [!IMPORTANT]
> 如果您的 Outlook 外接程序支持移动，请按照本文中针对 web、Windows 或 Mac 上的 Outlook 客户端的说明旁加载清单，然后按照[Outlook Mobile](outlook-mobile-addins.md#testing-your-add-ins-on-mobile)外接程序一文的移动测试外接程序部分中的指导操作。

## <a name="sideload-automatically"></a>自动旁加载

如果使用适用于 Outlook 加载项的[Yeoman](https://github.com/OfficeDev/generator-office)生成器创建了 Office 加载项，则最好通过 Windows 上的命令行进行旁加载。 这将利用我们的工具和通过一个命令跨所有受支持的设备进行旁加载。
...
在加载项中旁加载Outlook 网页版取决于使用的是新版本还是经典版本。
...
1. 选择功能区上的“**获取外接程序**”按钮。
    ...
    >   或
    >
    > - **"** 文件"菜单 **，然后选择"** 信息"选项卡上的"管理外接程序"按钮，以在Outlook 网页版 中打开"外接程序"对话框。<br>有关 Web 体验的更多内容，请参阅上一部分在加载项中旁[加载Outlook 网页版。](#outlook-on-the-web)
