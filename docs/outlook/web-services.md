...
EWS SOAP 请求完成后，Outlook 将使用一个实参（是一个 [AsyncResult](/javascript/api/office/office.asyncresult) 对象）调用该回调方法。 回调方法`AsyncResult``value`可以访问对象的两个属性：包含 EWS `asyncContext` `userContext` 操作 XML SOAP 响应的属性和（可选）属性（其中包含作为参数传递的任何数据）。 通常，回调方法稍后会解析 SOAP 响应中的 XML 以获取所有相关信息，并相应地处理这些信息。
...
- 使用 DOM 方法时指定标记名称的前缀 `getElementsByTagName`，以包含对Internet Explorer。
  ...
  `getElementsByTagName` 根据浏览器类型，其行为会有所不同。 例如，EWS 响应可以包含以下 XML (进行格式化和缩写，以便) 。
   ...
   如下文所示，代码将在 Chrome 等浏览器上运行，以将 XML 包含在 `ExtendedProperty` 标记中。
...
- 使用 DOM 属性 `textContent` 获取 EWS 响应中标记的内容，如下所示。
   ...
   其他属性（如 `innerHTML` ）可能Internet Explorer EWS 响应中某些标记的标记。
...
- `getSubjectRequest`&ndash;将项目 ID 作为输入，并返回 SOAP 请求的 XML，以调用`GetItem`指定项。

- `sendRequest`&ndash;调用 `getSubjectRequest` 获取选定项目的 SOAP 请求，然后传递 SOAP `callback``makeEwsRequestAsync` 请求和回调方法 ，获取指定项目的主题。
...
Outlook外接程序可以通过 方法访问 EWS 中可用的操作`makeEwsRequestAsync`子集。 如果您不熟悉 EWS `makeEwsRequestAsync` 操作以及如何使用 方法访问操作，请从 SOAP 请求示例开始自定义 _数据_ 参数。
...
使用 方法 `makeEwsRequestAsync` 时，将使用当前用户的电子邮件帐户凭据对请求进行身份验证。 方法 `makeEwsRequestAsync` 为您管理凭据，这样您就不需要随请求一起提供身份验证凭据。

> [!NOTE]
> 服务器管理员必须使用 [New-WebServicesVirtualDirectory](/powershell/module/exchange/client-access-servers/New-WebServicesVirtualDirectory?view=exchange-ps&preserve-view=true) 或 [Set-WebServicesVirtualDirectory](/powershell/module/exchange/client-access-servers/Set-WebServicesVirtualDirectory?view=exchange-ps&preserve-view=true) cmdlet 在客户端访问服务器 EWS `makeEwsRequestAsync` 目录上将 _OAuthAuthentication_ 参数设置为 **true**，才能允许该方法提出 EWS 请求。

外接程序必须在其外接程序 `ReadWriteMailbox` 清单中指定权限才能使用 `makeEwsRequestAsync` 方法。 有关使用权限的信息`ReadWriteMailbox`，请参阅了解加载项Outlook中的 [ReadWriteMailbox](understanding-outlook-add-in-permissions.md#readwritemailbox-permission) [权限部分](understanding-outlook-add-in-permissions.md)。
