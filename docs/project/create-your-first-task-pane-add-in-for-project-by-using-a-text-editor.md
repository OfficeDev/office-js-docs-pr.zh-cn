...
可以通过使用 Visual Studio 2015 创建复杂 Web 应用程序或使用文本编辑器创建本地加载项文件来创建 Project Standard 2013 或 Project Professional 2013 的任务窗格加载项。本文介绍了如何使用指向文件共享上的 HTML 文件的 XML 清单来创建简单的加载项。Project OM Test 示例加载项测试一些 JavaScript 功能，这些功能使用加载项对象模型。使用 Project 2013 中的“**信任中心**”注册包含清单文件的文件共享后，可以从功能区“**项目**”选项卡打开任务窗格加载项。（本文中的示例代码基于微软公司 Arvind lyer 所做的测试应用程序。）

Project 2013 使用与其他 Microsoft Office 2013 客户端相同的加载项清单架构，以及大部分相同的 JavaScript API。可从 Project 2013 SDK 下载的 `Samples\Apps` 子目录获取本文所述加载项的完整代码。

Project OM Test 示例加载项可以获取任务 GUID 和应用程序以及活动项目的属性。如果 Project Professional 2013 打开 SharePoint 库中的项目，加载项可以显示项目的 URL。 

[Project 2013 SDK 下载](https://www.microsoft.com/download/details.aspx?id=30435%20) 包含完整的源代码。提取和安装 Project2013SDK.msi 文件中的 SDK 和示例时，请在 `\Samples\Apps\Copy_to_AppManifests_FileShare` 子目录中查找清单文件，并在 `\Samples\Apps\Copy_to_AppSource_FileShare` 子目录中查找源代码。 

JSOMCall.html 示例在所包含的 office.js 文件和 project-15.js 文件中使用了 JavaScript 函数。可以使用相应的调试文件 （office.debug.js 和 project-15.debug.js） 检查这些函数。
...
在本地目录中创建 XML 文件。此 XML 文件包括 [Office 加载项 XML 清单](../develop/add-in-manifests.md)中描述的 **OfficeApp** 元素和子元素。例如，创建一个包含以下 XML 名为 JSOM_SimpleOMCalls.xml 的文件（更改 **Id** 元素的 GUID 值）。
...
对于 Project，**OfficeApp** 元素必须包括 `xsi:type="TaskPaneApp"` 属性值。**Id** 元素是 GUID。**SourceLocation** 值必须是任务窗格中运行的加载项 HTML 源文件或 Web 应用程序的文件共享路径或 SharePoint URL。有关清单文件中其他元素的说明，请参阅 [Project 任务窗格加载项](../project/project-add-ins.md)。
...
1. 创建 HTML 文件，名称通过 JSOM_SimpleOMCalls.xml 清单中的 **SourceLocation** 元素指定。 
   ...
   在 Microsoft Office 2013 应用程序中， JSOMCall.html 文件使用通用 MicrosoftAjax.js 文件实现 AJAX 功能并使用 Office.js 文件实现加载项功能。
   ...
   **textarea** 元素指定显示 JavaScript 函数结果的文本框。
   ...
   > [!NOTE]
   > 为了让“Project OM 测试”示例能够正常运行，请将 Project 2013 SDK 下载中的下列文件复制到 JSOMCall.html 文件所在的相同目录：Office.js、Project-15.js 和 MicrosoftAjax.js。
...
2. 在 JSOMCall.html 文件所在的相同目录中，创建 JavaScript 文件 JSOM_Sample.js。 
   ...
   下面的代码使用 Office.js 文件中的函数，获取应用程序上下文和文档信息。**text** 对象是 HTML 文件中 ** textarea** 控件的 ID。
    
   **\_projDoc** 变量使用 **ProjectDocument** 对象进行初始化。代码包含一些简单的错误处理函数，以及获取应用程序上下文和项目文档上下文属性的 **getContextValues** 函数。若要详细了解 Project 的 JavaScript 对象模型，请参阅[适用于 Office 的 JavaScript API](https://docs.microsoft.com/office/dev/add-ins/reference/javascript-api-for-office?view=office-js)。
   ...
   有关 Office.debug.js 文件中函数的信息，请参阅 [适用于 Office 的 JavaScript API](https://docs.microsoft.com/office/dev/add-ins/reference/javascript-api-for-office?view=office-js)。例如，**getDocumentUrl** 函数获取打开项目的 URL 或文件路径。
...
3. 添加调用 Office.js 和 Project-15.js 中异步函数的 JavaScript 函数，以获取选定数据：
   ...
   - Project-15.js 中的 **getSelectedTaskAsync** 函数用于获取选定任务的 GUID。同样，**getSelectedResourceAsync** 函数用于获取选定资源的 GUID。如果在未选择任务或资源时调用这些函数，函数将显示未定义错误。
   ...
   - **getTaskAsync** 函数用于获取任务名称和已分配资源的名称。如果任务位于同步的 SharePoint 任务列表中，**getTaskAsync** 可获取 SharePoint 列表中的任务 ID；否则，SharePoint 任务 ID 为 0。
     ...
     > [!NOTE]
     > 为了方便本文演示，示例代码有 bug。如果 **taskGuid** 未定义，**getTaskAsync** 函数不显示错误。如果获得有效的任务 GUID，并接着选择其他任务，**getTaskAsync** 函数会获取 **getSelectedTaskAsync** 函数最近一次处理的任务的数据。
   ...
   - 如果项目与 SharePoint 任务列表同步，则 **getWSSUrlAsync** 函数获取任务列表的 URL 和名称。如果项目不与 SharePoint 任务列表同步，则 **getWSSUrlAsync** 函数关闭错误显示。
...
12. 选择“**获取任务字段**”按钮。**getTaskFields** 函数会多次调用 **getTaskfieldAsync** 函数，以获取任务名称、索引、开始日期、持续时间、优先级和任务备注。
    ...
    - 持续时间： `4d`
    - 优先级： `500`
    - 备注： 这是任务 T2 的备注。只是一个测试备注。若为实际备注，应包含一些真实信息。

13. 选择 **getWSSUrlAsync** 按钮。如果项目是以下类型之一，结果将显示任务列表 URL 和名称。
...
使用 Visual Studio 2015 会更容易开发加载项，你可以在其中设置断点以帮助调试 JavaScript 代码并快速集成错误处理的公共例程。例如，Project 2013 SDK 下载中的 **HelloProject_OData** 示例包括使用 JQuery 库以显示弹出错误消息的 SurfaceErrors.js 文件。图 4 显示“toast”通知中的错误消息。该示例还包括 Office-vsdoc.js 文件，该文件提供对 Office.js 文件和 Project-15.js 文件的 JavaScript 函数的智能感知。
...
> [!NOTE]
> 部署加载项之前，请将 office.js 引用和 jQuery 引用更改为内容分发网络 (CDN) 引用。CDN 引用可提供最新的版本和更好的性能。
...
- [Office 加载项的 JavaScript API](https://docs.microsoft.com/office/dev/add-ins/reference/javascript-api-for-office?view=office-js)
