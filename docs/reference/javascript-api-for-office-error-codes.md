...
|2008|数据写入错误|TBD|TBD|
|2009|数据写入错误|指定的数据对象太大。|用户尝试设置由 Office 应用程序定义的数据限制之外的数据。|
...
|3010|不受支持的绑定操作|所选内容需为表格格式。将数据格式化为表格并再次尝试。|开发人员尝试对 `addRowsAsynch` `deleteAllDataValuesAsynch` `TableBinding` 强制类型 _matrix_的数据使用对象的或方法。|
