...
|3010|不受支持的绑定操作|所选内容需为表格格式。将数据格式化为表格并再次尝试。|开发人员尝试对 `addRowsAsynch` `deleteAllDataValuesAsynch` `TableBinding` 强制类型_matrix_的数据使用对象的或方法。|
...
|:-----|:-----|:-----|
|Matrix|文本|无法创建绑定。|
|矩阵|整张表格|_matrix_ 类型绑定已创建。文档已更改，内容控件必须打包表格。 |
|矩阵|表格范围内|无法创建绑定。|
|矩阵|选择无效（例如，多个对象、无效对象等。）|无法创建绑定。|
|Table|文本|无法创建绑定。|
