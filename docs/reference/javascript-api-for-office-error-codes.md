...
本文记录在使用 Office JavaScript API ( # A0) 时可能遇到的错误消息。
...
|:-----|:-----|:-----|:-----|
|1000|强制类型无效|不支持指定的强制类型|强制类型在 Office 应用程序中不受支持。  (例如，Excel 中不支持 OOXML 和 HTML 强制类型。 ) |
...
|1008|数据读取错误|请求的数据集太大。|用户请求读取超出由 Office 应用程序定义的数据限制的数据。|
...
|2008|数据写入错误|TBD|TBD|
|2009|数据写入错误|指定的数据对象太大。|用户尝试设置超出由 Office 应用程序定义的数据限制的数据。|
...
|3010|不受支持的绑定操作|所选内容需为表格格式。将数据格式化为表格并再次尝试。|开发人员尝试对 `addRowsAsync` `deleteAllDataValuesAsync` `TableBinding` 强制类型 _matrix_的数据使用对象的或方法。|
|4000|读取设置错误|指定设置名称不存在。|提供了不存在的设置名称。|
|4001|保存设置错误|无法保存设置。|无法保存设置。|
|4002|设置过期错误|无法保存设置，因为设置已过期。|设置已过期，开发人员指示不要覆盖设置。|
|5000|设置过期错误|不支持此操作。|当前 Office 应用程序不支持该操作。 例如， `document.getSelectionAsync` 从 Outlook 调用。|
...
|5004|无效的 API 调用|当前上下文中无效的 API 调用。|对上下文发出无效的调用，例如，尝试 `CustomXMLPart` 在 Excel 中使用对象。|
...
|9043|不支持的附件类型|不支持该附件类型。|API 不支持附件类型。 例如， `item.getAttachmentContentAsync` 如果附件是 Rtf 格式的嵌入图像，或者如果是除电子邮件或日历项目之外的项目类型 (（例如联系人或任务项) ），则会引发此错误。|
