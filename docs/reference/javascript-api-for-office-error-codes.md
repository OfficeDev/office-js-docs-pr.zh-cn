...
|1008|数据读取错误|请求的数据集太大。|用户请求读取超出应用程序定义的数据Office数据。|
...
|5000|设置过期错误|不支持此操作。|当前应用程序不支持该操作Office。 例如， `document.getSelectionAsync` 从 Outlook。|
...
|5004|无效的 API 调用|当前上下文中无效的 API 调用。|对上下文进行了无效调用，例如，尝试在 `CustomXMLPart` Excel。|
...
|6000|无效节点|未找到指定节点。|`CustomXmlPart`未找到节点。|
...
|9043|不支持附件类型|不支持附件类型。|API 不支持附件类型。 例如，如果附件是格式文本格式的嵌入图像，或者它是电子邮件或日历项目项目类型外的项目类型（如联系人或任务项目 (，则会引发此错误 `item.getAttachmentContentAsync`) 。|
