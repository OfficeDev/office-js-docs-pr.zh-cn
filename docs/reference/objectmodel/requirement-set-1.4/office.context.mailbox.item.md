...
`item` 命名空间用于访问当前选定的邮件、会议请求或约会。可以通过使用 [itemType](#itemtype-officemailboxenumsitemtypejavascriptapioutlook14officemailboxenumsitemtype) 属性确定 `item` 的类型。
...
获取对象，该对象提供用于获取或更新邮件的密件抄送 (Bcc) 行的方法。 仅限撰写模式。
