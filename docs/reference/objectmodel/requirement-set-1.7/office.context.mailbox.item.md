...
`organizer` 属性返回[组织者](/javascript/api/outlook_1_7/office.organizer)对象，该对象提供获取组织者值的方法。
...
如果项目是一个系列或系列中的一个实例，则 `recurrence` 属性将返回定期约会的[定期](/javascript/api/outlook_1_7/office.recurrence)对象或会议请求。 针对单个约会和单个约会的会议请求返回 `null`。 针对非会议请求的邮件返回 `undefined`。

> 注意：会议请求的 `itemClass` 值为 IPM.Schedule.Meeting.Request。

> 注意：如果定期对象为 `null`，则这表示对象是单个约会或单个约会的会议请求，而不是系列的一部分。

##### <a name="type"></a>类型：

* [定期](/javascript/api/outlook_1_7/office.recurrence)
...
#### <a name="nullable-seriesid-string"></a>（可以为 null）系列 Id：字符串
