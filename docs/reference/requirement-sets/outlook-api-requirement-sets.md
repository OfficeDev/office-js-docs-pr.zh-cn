...
Outlook 加载项通过在其[清单](https://docs.microsoft.com/office/dev/add-ins/develop/add-in-manifests)中使用 [Requirements](/office/dev/add-ins/reference/manifest/requirements) 元素来声明所需要的 API 版本。Outlook 加载项始终包括 [Set](/office/dev/add-ins/reference/manifest/set) 元素，其中 `Name` 属性设置为 且 `Mailbox` 且 `MinVersion`  属性设置为支持加载项方案的 API 最低要求集。
...
在清单中设置最低要求集版本可控制加载项会显示在哪个 Outlook 客户端中。如果客户端不支持最低要求集，则不会加载加载项。例如，如果指定要求集版本 1.3，则意味着加载项不会显示在任何不支持 1.3 及以上版本的 Outlook 客户端中。

## <a name="using-apis-from-later-requirement-sets"></a>使用更高版本要求集中的 API

设置要求集不会限制加载项可使用的可用 API。例如，如果加载项指定要求集 1.1，但它在支持 1.3 的 Outlook 客户端中运行，则加载项可以使用要求集 1.3 中的 API。
...
| Outlook Web 应用 （本地 Exchange 2013） | [1.1](/office/dev/add-ins/reference/objectmodel/requirement-set-1.1/outlook-requirement-set-1.1) |
| Outlook Web 应用 （本地 Exchange 2016） | [1.1](/office/dev/add-ins/reference/objectmodel/requirement-set-1.1/outlook-requirement-set-1.1)、[1.2](/office/dev/add-ins/reference/objectmodel/requirement-set-1.2/outlook-requirement-set-1.2)、[1.3](/office/dev/add-ins/reference/objectmodel/requirement-set-1.3/outlook-requirement-set-1.3) |

> [!NOTE]
> 对 Outlook 2013 中的 1.3 版本的支持已作为 [2015 年 12 月 8 日 Outlook 2013 更新 (KB3114349) ](https://support.microsoft.com/kb/3114349)的一部分添加。对 Outlook 2013 中的 1.4 版本的支持已作为 [2016 年 9 月 13 日 Outlook 2013 更新 (KB3118280) ](https://support.microsoft.com/help/3118280)的一部分添加。
