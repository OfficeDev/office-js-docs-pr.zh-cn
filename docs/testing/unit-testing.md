...
1. 项目将具有一个或多个测试文件。  (请参阅下面的示例 (#examples) 中的测试框架说明和测试文件示例。) 将库与 或 关键字一起导入到具有调用 Office `require` JavaScript API 的函数测试的任何测试文件，如以下示例所示。 `import`
...
此示例假定Office通用 API 的任何主机的 Office [Office](../develop/office-javascript-api-object-model.md)外接程序 (例如 Excel、PowerPoint 或 Word) 。 加载项在名为 的文件中有一项功能 `my-common-api-add-in-feature.js` 。 下面显示了文件的内容。 函数 `addHelloWorldText` 设置文本"Hello World！" 为文档中当前选择的任何内容;例如;Word 中的区域，或 Excel 中的单元格，或 PowerPoint。
...
- 构造函数不会将所有 Office枚举类添加到 mock 对象，因此外接程序方法中引用的值必须显式添加到 `OfficeMockObject`  `Office` seed `CoercionType.Text` 对象中。
...
尽管严格来说，Outlook API 是通用 API 模型的一部分，但是它们具有围绕[Mailbox](/javascript/api/outlook/office.mailbox)对象构建的特殊体系结构，因此我们为 Outlook 提供了一个Outlook。 此示例假定一Outlook，该文件在名为 的文件中具有其功能之一 `my-outlook-add-in-feature.js` 。 下面显示了文件的内容。 函数 `addHelloWorldText` 设置文本"Hello World！" 为当前在邮件撰写窗口中选择的任何内容。
...
此示例假定Excel一个外接程序，该加载项在名为 的文件中具有其一个功能 `my-excel-add-in-feature.js` 。 下面显示了文件的内容。 请注意， 是在传递给 的回调内 `getSelectedRangeAddress` 调用的帮助程序方法 `Excel.run` 。
