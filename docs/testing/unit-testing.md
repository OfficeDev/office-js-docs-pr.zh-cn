...
本文中的示例使用 Jest 框架。 在[Office-Addin-Mock 主页上提供了使用](https://github.com/OfficeDev/Office-Addin-Scripts/tree/master/packages/office-addin-mock#examples)Mocha 框架的示例。
...
> [!TIP]
> 如果你使用 Visual Studio，我们建议你阅读 Visual Studio 中的单元测试[JavaScript 和 TypeScript](/visualstudio/javascript/unit-testing-javascript-with-visual-studio)一文，了解一些有关 Visual Studio 中的 JavaScript 单元测试的基本信息，然后返回到本文。
...
1. 项目将具有一个或多个测试文件。  (请参阅下面的示例 (#examples) 中的测试框架说明和示例测试文件。) 将库（带 或 关键字）导入到具有调用 Office `require` JavaScript API 的函数测试的任何测试文件，如以下示例所示。 `import`
...
此示例假定Office通用 API 的任何主机的 Office 加载项 (例如 Excel、PowerPoint 或 Word) 。 [](../develop/office-javascript-api-object-model.md) 加载项在名为 的文件中有一项功能 `my-common-api-add-in-feature.js` 。 下面显示了文件的内容。 函数 `addHelloWorldText` 设置文本"Hello World！" 为文档中当前选择的任何内容;例如;Word 中的区域、Excel中的单元格或 PowerPoint。
...
名为 的测试 `my-common-api-add-in-feature.test.js` 文件位于子文件夹（相对于加载项代码文件的位置） 中。 下面显示了文件的内容。 请注意，顶级属性是 `context` [，Office。Context](/javascript/api/office/office.context)对象，因此要模拟的对象是此属性的父对象：Office[对象。](/javascript/api/office) 关于此代码，请注意以下几点：
...
名为 的测试 `my-outlook-add-in-feature.test.js` 文件位于子文件夹（相对于加载项代码文件的位置） 中。 下面显示了文件的内容。 请注意，顶级属性是 `context` [，Office。Context](/javascript/api/office/office.context)对象，因此要模拟的对象是此属性的父对象：Office[对象。](/javascript/api/office) 关于此代码，请注意以下几点：
