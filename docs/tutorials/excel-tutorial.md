...
2. 打开文件 **./src/taskpane/taskpane.html**。该文件包含任务窗格的 HTML 标记。

3. 找到 `<main>` 元素并删除在开启 `<main>` 标记后和关闭 `</main>` 标记前出现的所有行。
...
5. 打开文件 **./src/taskpane/taskpane.js**。该文件包含用于加快任务窗格和 Office 客户端应用程序之间交互的 Office JavaScript API 代码。
...
7. 在 `Office.onReady` 方法调用中，找到 `if (info.host === Office.HostType.Excel) {` 一行，并紧贴该行后添加以下代码：
...
8. 将以下函数添加到文件结尾。注意：
    ...
    - Excel.js 业务逻辑将添加到传递给 `Excel.run` 的函数。 此逻辑不会立即执行。 相反，它会被添加到挂起的命令队列中。
...
9. 在 `createTable()` 函数中，使用以下代码替换 `TODO1`。注意：

    - 此代码通过使用工作表的表格集合的 `add` 方法来创建表格，该表即使是空表也始终存在。 这是创建 Excel.js 对象的标准方式。 没有类构造函数 API，切勿使用 `new` 运算符创建 Excel 对象。 相反，请添加到父集合对象。
...
10. 在 `createTable()` 函数中，使用以下代码替换 `TODO2`。注意：

    - 范围的单元格值是通过一组数组进行设置。

    - 表格中的新行通过调用表格的行集合的 `add` 方法进行创建。 通过在作为第二个参数传递的父数组中添加多个单元格值数组，可以在一次 `add` 调用中添加多个行。
...
11. 在 `createTable()` 函数中，使用以下代码替换 `TODO3`。注意：

    - 此代码通过将从零开始编制的索引传递给表格列集合的 `getItemAt` 方法，来获取对 **Amount** 列的引用。
...
1. 完成以下步骤，以启动本地 Web 服务器并旁加载你的加载项。
    ...
    > [!NOTE]
    > 即使正在研发，Office 加载项将使用 HTTPS，而非 HTTP。如果系统在你运行以下命令之一后提示安装证书，请接受提示，安装 Yeoman 生成器提供的证书。

    > [!TIP]
    > 如果在 Mac 上测试加载项，请先在项目的根目录中运行以下命令，然后再继续操作。运行该命令时，本地 web 服务器将会启动。
    >
    > ```command&nbsp;line
    > npm run dev-server
    > ```

    - 若要在 Excel 中测试加载项，请在项目的根目录中运行以下命令。此操作将启动本地 web 服务器（如果服务器尚未运行），并在加载后运行 Excel。

        ```command&nbsp;line
        npm start
        ```

    - 若要在 Excel 网页版中测试加载项，请在项目的根目录中运行以下命令。运行该命令时，本地 web 服务器将会启动（如果服务器尚未运行）。
...
6. 在 `filterTable()` 函数中，使用以下代码替换 `TODO1`。注意：

   - 代码先将列名称传递给 `getItem` 方法（而不是像`createTable` 方法一样将列索引传递给 `getItemAt` 方法），获取对需要筛选的列的引用。 由于用户可以移动表格列，因此给定索引处的列可能会在表格创建之后更改。 所以，更安全的做法是，使用列名称获取对列的引用。 上一教程安全地使用了`getItemAt`，因为是在与创建表格完全相同的方法中使用了它，所以用户没有机会移动列。
...
6. 在 `sortTable()` 函数中，使用以下代码替换 `TODO1`。注意：

   - 此代码创建一组 `SortField` 对象，其中只有一个成员，因为加载项只对“商家”列进行了排序。

   - `SortField` 对象的`key` 属性是用于排序的列从零开始编制的索引。表格中的行按照所引用列的值进行排序。

   - `Table` 的 `sort` 成员是 `TableSort` 对象，而不是方法。`SortField`s 被传递到`TableSort` 对象的 `apply` 方法。
...
6. 在 `createChart()` 函数中，使用以下代码替换 `TODO1`。请注意，为了排除标题行，代码使用 `Table.getDataBodyRange` 方法（而不是`getRange`方法）获取要绘制成图表的数据的范围。
...
7. 在 `createChart()` 函数中，使用以下代码替换`TODO2`。请注意以下参数：

   - `add` 方法的第一个参数指定图表类型。图表有几十种类型。

   - 第二个参数指定要在图表中添加的数据的范围。

   - 第三个参数确定按行方向还是按列方向绘制表格中的一系列数据点。选项 `auto` 告诉 Excel 选择最佳方法。

    ```js
    var chart = currentWorksheet.charts.add('ColumnClustered', dataRange, 'auto');
    ```

8. 在 `createChart()` 函数中，使用以下代码替换 `TODO3`。此代码大多无需解释。请注意：

   - `setPosition` 方法的参数指定应包含图表的工作表区域左上角和右下角单元格。 Excel 可以调整行宽等设置，以便图表能够适应所提供的空间。
...
6. 在 `freezeHeader()` 函数中，使用以下代码替换 `TODO1`。注意：
...
2. 找到 `<Control>` 元素。此元素定义了用于启动加载项的 **Home** 功能区上 **"Show Taskpane"** 按钮。接下来，我们将在 **Home** 功能区的同一组中添加另一个按钮。在结束 `</Control>` 标记和 结束 `</Group>` 标记之间，添加以下标记。
...
3. 在刚刚添加到清单文件的 XML 中，用一个字符串替换 `TODO1`，该字符串为此该按钮提供一个此清单文件内唯一的 ID。因为这个按钮将开启或关闭工作表的保护状态，所以请使用“ToggleProtection”。操作完成后，`Control` 元素的开始标记应如下所示：

    ```xml
    <Control xsi:type="Button" id="ToggleProtection">
    ```

4. 以下三个 `TODO` 设置资源 ID 或 `resid`。资源是一个字符串，在后续步骤中，将创建这三个字符串。按钮标签应显示为“Toggle Protection”，但字符串的 *ID* 应当显示为“ProtectionButtonLabel”，因此，`Label` 元素应如下所示：

    ```xml
    <Label resid="ProtectionButtonLabel" />
    ```

5. `SuperTip`元素定义按钮的工具提示。工具提示标题应与按钮标签相同，因此我们使用的资源 ID 完全相同： “ProtectionButtonLabel”。工具提示说明将为“单击打开或关闭工作表保护”。但 `resid` 应该是“ProtectionButtonToolTip”。因此，操作完成后，`SuperTip` 元素应如下所示：
   ...
   > [!NOTE]
   > 在生产加载项中，你可能不希望对两个不同的按钮使用相同的图标；但为简单起见，本教程将采用这样的做法。 因此，新 `Control`的`Icon` 标记就是现有 `Control`中`Icon` 元素的副本。

6. 原始 `Control` 元素中的 `Action` 元素的类型设置为 `ShowTaskpane`，但我们的“新建”按钮不会打开任务窗格，而是运行在后面的步骤中创建的自定义函数。因此，请用 `ExecuteFunction`替换 `TODO5`，后者是触发自定义函数的按钮的操作类型。`Action` 元素的开始标记应如下所示：

    ```xml
    <Action xsi:type="ExecuteFunction">
    ```

7. 原始 `Action` 元素的子元素指定任务窗格 ID，以及应当在任务窗格中打开的页面 URL。不过，`ExecuteFunction` 类型的 `Action` 元素只有一个子元素，用于命名控件执行的函数。此函数（名为 `toggleProtection`）将在后续步骤中创建。因此，请用以下标记替换 `TODO6`：
...
2. 紧随 `action` 函数之后添加以下函数。请注意，我们为此函数和此函数调用的 `args.completed`指定了`args`参数。这是对所有 **ExecuteFunction** 类型的加载项命令的要求。它向 Office 客户端应用程序发出以下信号：该函数已完成，UI 可能会再次迅速响应。
...
4. 在 `toggleProtection` 函数中，使用以下代码替换 `TODO1`。此代码使用处于标准切换模式的工作表对象的保护属性。有关`TODO2` 的说明，请参阅下一节。
...
在到目前为止本教程里所创建的每个函数中，你将命令排队 *写入* 到 Office 文档。每个函数都以对 `context.sync()` 方法的调用作为结尾，这将会把排队的指令发送到要执行的文档。但是，在上一步中添加的代码会调用 `sheet.protection.protected property`。这一点与之前编写的函数存在明显不同，因为 `sheet` 对象仅是存在于任务窗格脚本中的代理对象。代理对象不知道文档的实际保护状态，所以它的 `protection.protected` 属性不可能是真实值。若要避免异常错误，必须从文档中提取保护状态，并用它设置 `sheet.protection.protected`的值。提取过程分为三步：
...
1. 在 `toggleProtection` 函数中，使用以下代码替换 `TODO2`。注意：
   ...
   - 每个 Excel 对象都有 `load` 方法。 对于要在参数中读取的对象属性，将它们指定为逗号分隔名称字符串。 在此示例中，需要读取的属性为 `protection` 属性的子属性。 引用子属性的方法与代码中其他任何地方的引用属性几乎完全一样，不同之处在于使用的是正斜杠（“/”）字符，而不是“.”字符。
...
1. 关闭包括 Excel 在内的所有 Office 应用程序。
...
3. 如果本地 web 服务器已在运行，通过关闭节点命令窗口将其停止。

4. 因为清单文件已经更新，必须使用已更新的清单文件再次旁加载你的加载项。启动本地 web 服务器，旁加载你的加载项：
...
5. 在 Excel 的 **Home** 选项卡上，选择 **切换工作表保护** 按钮，如下面的屏幕截图所示。请注意，功能区上的多数控件都处于禁用状态（灰色显示）。
...
2. 在 **./src/dialogs** 文件夹中，新建名为 **popup.html** 的文件。

3. 将以下标记添加到 **popup.html** 中。注意：

   - 此页面包含可供用户输入用户名的 `<input>` 字段，以及将此名称发送到将在其中显示的任务窗格的按钮。
...
4. 在 **./src/dialogs** 文件夹中，新建名为 **popup.js** 的文件。

5. 将下列代码添加到 **popup.js**。注意此代码的以下信息：

   - *调用 Office.js 库中 API 的每个页面都必须首先确保库已完全初始化。* 执行此操作的最佳方式是调用 `Office.onReady()` 方法。如果加载项具有自己的初始化任务，代码应当采用连接到 `Office.onReady()` 调用的 `then()` 方法。`Office.onReady()` 调用必须在调用 Office.js 之前运行；因此，作业位于由页面加载的脚本文件中，如下例所示。
...
7. 使用以下代码替换 `TODO2`。`messageParent` 方法将它的参数传递到父页面（在此示例中，为任务窗格中的页面）。 参数可以是布尔值或字符串，其中包含可串行化为字符串的任何内容（如 XML 或 JSON）。
...
> [!NOTE]
> **popup.html** 文件及其所加载的 **popup.js** 文件在完全独立于加载项任务窗格之外的 Microsoft Edge 或 Internet Explorer 11 过程中运行。如果 **popup.js** 被编译到与 **app.js** 文件相同的 **bundle.js** 文件中，加载项必须加载 **bundle.js** 文件的两个副本，这将使捆绑目的无法实现。因此，此加载项绝对不会编译 **popup.js** 文件。
...
2. 找到 `freeze-header` 按钮的 `<button>` 元素，并在行后添加下列标记：

    ```html
    <button class="ms-Button" id="open-dialog">Open Dialog</button><br/><br/>
    ```

3. 该对话框将提示用户输入名称，并将用户名称传递到任务窗格。任务窗格将在标签中显示该名称。紧贴刚添加的 `button` 之后，添加以下标记：
...
5. 在 `Office.onReady` 方法调用中，定位分配单机处理程序至 `freeze-header` 按钮的行，并在行后添加以下代码。将在后续步骤中创建 `openDialog` 方法。
...
7. 在文件结尾（在 `dialog` 的声明之后）添加以下函数。关于这一代码的重要之处在于是 *不是*：没有 `Excel.run`的调用。这是因为打开对话框的 API 在全部 Office 应用程序间共享，所以它属于 Office JavaScript 通用 API，而不是 Excel 专用 API。
...
5. 在对话框中，输入一个名称，然后选择 **确定** 按钮。该名称将显示在任务窗格中，且对话框将关闭。

6. （可选）为 `processMessage` 函数中的行 `dialog.close();`添加备注。然后重复本节中的步骤。对话框将保持打开状态，你可以更改名称。可通过按右上角的 **X** 按钮将其手动关闭。

    ![Excel 的屏幕截图，在“加载项”任务窗格中显示“打开”对话框按钮，并在工作表上显示对话框](../images/excel-tutorial-dialog-open-2.png)

## <a name="next-steps"></a>后续步骤

在本教程中，你已创建与 Excel 工作簿中的表格、图表、工作表和对话框进行交互的 Excel 任务窗格加载项。若要了解创建 Excel加载项的更多信息，请继续阅读下面的文章：
